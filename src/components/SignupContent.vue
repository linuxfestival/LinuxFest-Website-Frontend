<template>
  <div class="back">
    ...
    <div class="main-frame">
      ...
      <form class="top" @submit.prevent="submitUser()">
        <input
          ...
          v-bind:class="!verifyFirstName ? 'notVerified' : 'input'"
          placeholder="نام"
        />
        ...
        <input
          ...
          v-bind:class="!verifyLastName ? 'notVerified' : 'input'"
        ...
        <input
          ...
          v-bind:class="!verifyEmail ? 'notVerified' : 'input'"
          placeholder="ایمیل"
        />
        ...
        <input
          ...
          v-if="this.user.isAmirkabiri"
          v-bind:class="!verifyStudentNumber ? 'notVerified' : 'input'"
          placeholder="شماره دانشجویی"
        />
        <p
          v-if="!verifyStudentNumber"
          class="red"
        ...
        <input
          ...
          v-bind:class="!verifyPhoneNumber ? 'notVerified' : 'input'"
          placeholder="تلفن"
        />
        <p
          v-if="!verifyPhoneNumber"
          class="red"
        ...
        <input
          ...
          v-bind:class="!verifyAge ? 'notVerified' : 'input'"
          placeholder="سن"
        />
        ...
        <input
          ...
          v-bind:class="!verifyPassword ? 'notVerified' : 'input'"
    ...
    <button @click="checkModel()">click</button>
  </div>
</template>
...
const persianPhoneValidator = helpers.regex('persianPhoneValidator', /^(\\+98|0)?9\\d{9}$/);
const passwordRegexValidator = helpers.regex('passwordValidator',/^(?=.*[a-z])(?=.*[A-Z])(?=.*d)[a-zA-Zd]{8,}$/);
const persianLetterValidator = helpers.regex('persianLetterValidator', /^[\u0600-\u06FF\s]+$/)
const booleanValidator = (value) => {return value === false || value === true};
export default {
  name: "SignupContent",
  validations: {
    user: {
      firstName: { required, persianLetterValidator },
      lastName: { required, persianLetterValidator },
      ...
      phoneNumber: { required, numeric, persianPhoneValidator, requiredIf: requiredIf(() => {return isAmirkabiri})},
  ...
  methods: {
    ...
    submitUser: function() {
      ...
      if (
        ...
        if (
          (this.checked === "" && this.user.studentNumber === "") ||
          (this.checked !== "" && this.user.studentNumber !== "")
        ) {
          if (
            ...
            for (var propName in this.user) {
              if (
                this.user[propName] === null ||
                this.user[propName] === "" ||
                this.user[propName] === undefined
              ) {
  ...
  computed: {
    ...
    verifyPhoneNumber: function() {
      ...
      if (
        this.user.phoneNumber == "" ||
        (this.user.phoneNumber.length == 11 && p.test(this.user.phoneNumber))
      ) {
        return true;
      } else {
        return false;
    ...
    verifyStudentNumber: function() {
      ...
      if (
        ...
        (this.user.studentNumber.length == 7 && p.test(this.user.studentNumber))
      ) {
        return true;
      } else {
        return false;
    ...
    verifyPassword: function() {
      ...
      if (
        this.user.password == "" ||
        (this.user.password.length > 5 && p.test(this.user.password))
      ) {
        return true;
      } else {
        return false;
