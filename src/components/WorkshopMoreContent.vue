<template>
  <div class="parent">
    <PartialHeader :title="getWorkShop.workshop.title"/>
    <div class="workshopContentWrapper">
      <div class="workshopContent">
        <div class="workshopTeachersDescription">
          <h2 v-if="getWorkShop.workshop.teachers.length === 1"
              class="workshopTeachersDescription-title">
          ...
          <div class="teacherDescriptionWrapper">
            <div class="teacherDescription" v-for="teacher in getWorkShop.teachers"
                 :key="getWorkShop.teachers.indexOf(teacher)">
        ...
        <div class="workshopTeachersImages">
          <img v-for="teacher in getWorkShop.teachers"
               :key="getWorkShop.teachers.indexOf(teacher)"
        ...
        <div class="workshopContentDescription">
          <h1 v-if="getWorkShop.workshop.price === 0"
              class="workshopContentDescription-title">
          ...
          <div class="workshopContentDescription-desc"
               v-html="getWorkShop.workshop.description"></div>

          <h3 class="timeline-title">جدول زمانی:</h3>

          <p class="timeline-description" v-for="timeSlot in getWorkShop.workshop.times">
            {{ getWorkShop.workshop.times.indexOf(timeSlot) + 1 }}.
          ...
          <p class="timeline-description" v-if="getWorkShop.workshop.price !== 0">
            {{ getWorkShop.workshop.price }}
            ریال
          </p>
          <p class="timeline-description" v-else>
            رایگان
          </p>
          <router-link :to="'/registerworkshop/?workshop=' + getWorkShop.workshop._id"
                       v-if="getWorkShop.workshop.isRegOpen" class="registerButton">ثبت نام
          </router-link>
...
export default {
  ...
  computed: {
    getWorkShop(){
      return this.$store.getters.getWorkshopMore
    }
  },
  async created() {
